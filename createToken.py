...
availableTerminal=[ 'False' , 'True'    , 'None'    , 'and'     , 'as'      , 'break', 
                    ...
                    'print' , 'string'  , 'integer' , 'float'   , 'range'   , 'content',
                    'input' , 'str'     , 'len'     , 'open'    , 'int'     , 'end=""']

def concatString1(content):
    startComment = []
    endComment = []
    count = 1
...
def concatString(content):       
    content = concatString1(content)
    content = concatString2(content)
    return content

def createToken(inputFilename):
    # Read from file
